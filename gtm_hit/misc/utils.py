...
def get_frame_path_dict(dset, frame_path_template, cams):
    from pathlib import Path
    # This dictionary will map the "common" frame number to a nested dict of {camera: frame file path}
    frame_dict = {}
    for cam in cams:
        # Build the directory path for this camera using the provided template.
        # The template is assumed to have placeholders {dset} and {cam}.
        cam_dir = Path(frame_path_template.format(dset=dset, cam=cam))
        if not cam_dir.exists():
            continue

        # Determine camera-specific offset based on the paster code.
        if cam == "cvlabrpi11":
            offset = 38
        elif cam == "cvlabrpi22":
            offset = 16
        ...
        for img_file in sorted(cam_dir.glob("*.jpg")):
            # Assume image filenames end with an underscore followed by the frame number,
            # e.g. "cvlabrpi11_someinfo_62.jpg"
            filename = img_file.stem  # Remove the file extension.
            parts = filename.split('_')
            try:
                file_frame_num = int(parts[-1])
            except ValueError:
                # If the last part is not a number, skip this file.
                continue

            # Compute the common frame number by adding the offset.
            common_frame = file_frame_num + offset

            # Ensure we have an entry for this common frame number.
            if common_frame not in frame_dict:
                frame_dict[common_frame] = {}
            frame_dict[common_frame][cam] = str(img_file)
    return frame_dict
